...
async function loadProviders() {
  ...
  try {
    ...
    console.warn('[qconsole.fx] NÃ£o foi possÃ­vel carregar providers.json, usando padrÃ£o.', error);
...
async function requestRates(providerName, baseUrl, base, symbols) {
  ...
  throw new Error(`Provedor FX nÃ£o suportado: ${providerName}`);
...
export async function fetchFX(base = 'USD', symbols = SUPPORTED, options = {}) {
  ...
  throw lastError || new Error('NÃ£o foi possÃ­vel obter taxas de cÃ¢mbio');
...
function ensureRates() {
  ...
  if (!latest) throw new Error('Taxas de cÃ¢mbio indisponÃ­veis');
...
export function convert(amount, from, to) {
  ...
  if (!normalizedRates.has(from) || !normalizedRates.has(to)) {
    ...
    throw new Error(`ConversÃ£o nÃ£o suportada: ${from} -> ${to}`);
...
function updateProviderMeta(providerElement, updatedElement) {
  ...
  if (providerElement) {
    ...
    providerElement.textContent = providers ? String(providers.fx) : 'â';
...
function renderConverterResults(resultsElement, amountInput, fromSelect) {
  ...
  if (!value || Number.isNaN(value) || value <= 0) {
    ...
    resultsElement.textContent = 'Digite um valor e selecione a moeda de origem para ver as conversÃµes em USD, BRL e ARS.';
  ...
  resultsElement.textContent = `${formatMoney(value, from)} equivalem a ${pieces.join(' â¢ ')}`;
...
export async function initFX(options = {}) {
  ...
  async function refresh(force = false) {
    ...
    try {
      ...
      console.error('[qconsole.fx] Falha ao atualizar cÃ¢mbio', error);


      if (ratesContainer) {


        ratesContainer.innerHTML = '<article class="fx-card"><p>NÃ£o foi possÃ­vel atualizar as taxas agora.</p><p class="panel-note">Verifique a rede ou altere o provedor em <code>data/providers.json</code>. Frankfurter.dev e ExchangeRate.host funcionam sem chave.</p></article>';
      ...
      if (resultsElement) {
        ...
        resultsElement.textContent = 'NÃ£o foi possÃ­vel atualizar agora. Ajuste o provedor em data/providers.json ou tente novamente em instantes.';
