...
const GROUP_LABELS = {
  ...
  reasoning: 'RaciocÃÂ­nio/Agentes',


  multimodal: 'Multimodal',


  'cheap-fast': 'Texto rÃÂ¡pido/custo baixo',


  'long-context': 'Contexto grande',


  'image-audio': 'Imagem/ÃÂudio',


};


const TOKENS_PER_PAGE = 750; // AproximaÃÂ§ÃÂ£o: pÃÂ¡gina de Word com ~500 palavras.
...
function formatPerPage(value, fx) {
  ...
  try {
    ...
    console.warn('[qconsole.models] Falha ao converter custo por pÃÂ¡gina', error);
...
function createPricingBlock(model, fx) {
  ...
  if (typeof usdInput === 'number' || typeof usdOutput === 'number') {
    ...
    if (typeof usdOutput === 'number') parts.push(`SaÃÂ­da: ${formatMoney(usdOutput, 'USD')} / 1M tokens`);
  ...
  if (typeof usdInput === 'number') {
    ...
    if (perPage) {
      ...
      const values = [
        ...
        `Entrada (~pÃÂ¡gina): ${formatMoney(perPage.usd, 'USD')}`,
      ...
      item.textContent = values.join(' Ã¢â¬Â¢ ');
  ...
  if (typeof usdOutput === 'number') {
    ...
    if (perPage) {
      ...
      const values = [
        ...
        `SaÃÂ­da (~pÃÂ¡gina): ${formatMoney(perPage.usd, 'USD')}`,
  ...
  if (typeof audioMinute === 'number') {
    ...
    if (audioCost) {
      ...
      const values = [
        ...
        `ÃÂudio/min: ${formatMoney(audioCost.usd, 'USD')}`,
  ...
  if (!wrapper.childElementCount) {
    ...
    empty.textContent = 'Custos nÃÂ£o informados.';
...
function createModelCard(model, index, fx, drawerRegistry) {
  ...
  vendorLabel.textContent = model.vendor || 'Ã¢â¬â';
  ...
  if (model.link) {
    ...
    link.innerHTML = 'Ver preÃÂ§o oficial <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true"><path d="M14 3h7v7h-2V6.41l-9.29 9.3-1.42-1.42 9.3-9.29H14V3z"/><path d="M5 5h5V3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-5h-2v5H5z"/></svg>';
  ...
  const tooltipContent = model.hover || 'Resumo nÃÂ£o informado.';
  ...
  const limitsFragment = sanitizeToFragment(`<strong>Limites:</strong> ${model.limits || 'Sem anotaÃÂ§ÃÂµes.'}`);
  ...
  if (model.context) {
    ...
    context.className = 'model-card__context';


    context.textContent = `Contexto mÃ¡ximo: ${model.context}`;
...
function renderGroups(container, models, fx, drawerRegistry) {
  ...
  GROUP_ORDER.forEach((groupKey) => {
    ...
    hint.className = 'models__group-hint';


    hint.textContent = 'Expanda um fornecedor para ver os modelos recomendados.';
    ...
    vendorNames.forEach((vendor) => {
      ...
      const vendorModels = Array.from(vendors.get(vendor) || []);
      ...
      const cardsId = `model-vendor-${groupKey}-${vendorIndex}`;
      ...
      toggle.className = 'models__vendor-toggle';


      toggle.setAttribute('aria-expanded', 'false');


      toggle.setAttribute('aria-controls', cardsId);
      ...
      label.className = 'models__vendor-label';
      ...
      meta.className = 'models__vendor-meta';


      const modelCount = vendorModels.length;


      meta.textContent = `${modelCount} modelo${modelCount === 1 ? '' : 's'}`;
      ...
      chevron.className = 'models__vendor-chevron';
      ...
      toggle.addEventListener('click', () => {
        ...
        block.dataset.state = willExpand ? 'expanded' : 'collapsed';


        cards.hidden = !willExpand;
...
export async function initModelDirectory({ container, drawer, fx } = {}) {
  ...
  function fillDrawerContent(modelIndex) {
    ...
    panelMapping.forEach(({ panel, key }) => {
      ...
      const safeContent = sanitizeToFragment(content || 'ConteÃÂºdo nÃÂ£o informado.');
  ...
  try {
    ...
    console.error('[qconsole.models] Falha ao carregar catÃÂ¡logo de modelos', error);


    container.innerHTML = '<p>NÃ£o foi possÃ­vel atualizar agora.</p>';
